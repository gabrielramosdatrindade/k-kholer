...
df = pd.read_csv('dadoscorretos.csv', names = ["Wet diameter","Supersaturation"])

fig = plt.figure(facecolor='w')
ax = fig.add_subplot(111, facecolor='w', axisbelow=True)
ax.plot(D_wet1/10**(-6), S2, 'k', alpha=1, lw=2, linestyle='dashed', label ='D = 50 nm kappa = 1,28')
ax.plot(D_wet2/10**(-6), S4, 'b', alpha=1, lw=2, linestyle='dashed', label ='D = 100 nm kappa = 1,28')
ax.plot(D_wet3/10**(-6), S6, 'r', alpha=1, lw=2, linestyle='dashed', label ='D = 500 nm kappa = 1,28')
ax.scatter(df["Wet diameter"], df["Supersaturation"], label = 'S&P')
ax.set_xlabel('Diâmetro úmido (nm)')
ax.set_ylabel('Supersaturação (%)')
ax.set_ylim(-0.3,0.4)
ax.set_xscale('log')
ax.set_title('NaCl', fontsize = 15)
